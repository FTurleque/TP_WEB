...
> Le script SQL fourni est standard et initialement prévu pour fonctionner avec MySQL. Adaptez-le au serveur de base de données que vous utiliserez pour ce projet. Par exemple, si vous utilisez Microsoft SQL Server, il est recommandé de renommer les tables/colonnes en `camelCase` - (ex: `customer_name` devient `customerName`).
